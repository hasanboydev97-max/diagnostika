        ...
        <AnimatePresence mode="wait">
          ...
          {gameState === 'stage_select' && (
            ...
            >
              <div className="text-center mb-8">
                <div className="w-20 h-20 bg-gradient-to-tr from-cyan-400 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-xl shadow-cyan-500/30 text-4xl border-2 border-white">
                  🌍
                </div>
                <h1 className="text-3xl md:text-4xl font-black tracking-tight text-white mb-2">Global Live Trivia Duel</h1>
                <p className="text-xs text-cyan-200 font-medium">Open Trivia API orqali jonli bilimlar va fanlar duelini yuting!</p>
              </div>
                  ...
                  return (
                    <div key={stage.level} className={`p-5 rounded-2xl border-2 transition-all flex items-center justify-between gap-4 ${isUnlocked ? 'bg-indigo-950/60 border-indigo-400/60 shadow-lg' : 'bg-slate-900/40 border-slate-800 opacity-60'}`}>
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg ${isUnlocked ? 'bg-cyan-400 text-slate-950 shadow-md shadow-cyan-400/30' : 'bg-slate-800 text-slate-500'}`}>
                          {isUnlocked ? stage.level : <Lock className="w-5 h-5" />}
                        </div>
                        <div>
                          <h3 className="font-extrabold text-sm text-white">{stage.name}</h3>
                          <p className="text-xs text-cyan-200 mt-0.5">{stage.targetQuestions} ta savol • Open Trivia DB API</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`px-5 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all flex items-center gap-2 border-b-4 ${isUnlocked ? 'bg-emerald-500 hover:bg-emerald-400 text-slate-950 border-emerald-700 active:border-b-0 cursor-pointer shadow-lg shadow-emerald-500/20' : 'bg-slate-800 text-slate-600 border-slate-900 cursor-not-allowed'}`}
                        >
          ...
          {gameState === 'loading' && (
            <motion.div key="loading" className="flex flex-col items-center justify-center py-16">
              ...
              <p className="font-extrabold text-white text-sm uppercase tracking-wider animate-pulse">Open Trivia API dan Jonli Savollar Yuklanmoqda...</p>
            </motion.div>
          )}
          ...
          {gameState === 'playing' && currentQ && (
            <motion.div key="playing" className="w-full flex flex-col items-center justify-between max-w-2xl">
              ...
              <div className="w-full flex items-center gap-3 mb-4 bg-slate-900/80 backdrop-blur-xl border border-white/15 p-3 rounded-2xl shadow-lg">
                <motion.div animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-12 h-12 bg-gradient-to-tr from-cyan-400 to-blue-500 rounded-2xl flex items-center justify-center text-2xl shadow-md border border-white">
                  🌍
                </motion.div>
                <div className="bg-cyan-950/60 border border-cyan-400/40 px-4 py-2 rounded-xl text-xs font-bold text-cyan-300">
                  {mascotQuote}
                </div>
              </div>
              ...
              <div className="w-full mb-4">
                <div className="bg-slate-900/90 backdrop-blur-xl border-2 border-indigo-500/40 rounded-2xl p-4 flex justify-between items-center shadow-xl mb-3">
                  <div className="flex gap-4 items-center">
                    <div className="font-black uppercase tracking-wider text-xs text-amber-300 flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-xl border border-white/10">
                      <Clock className="w-4 h-4 text-cyan-400 animate-spin" /> {timeLeft}S
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-black text-cyan-200 uppercase tracking-wider bg-slate-800 px-3 py-1 rounded-xl border border-white/10">
                ...
                <div className="w-full h-3 rounded-full border-2 border-white/20 bg-slate-950 overflow-hidden shadow-inner p-[1px]">
                  <motion.div className={`h-full rounded-full transition-all duration-300 ${timeLeft <= 8 ? 'bg-rose-500' : 'bg-cyan-400'}`} initial={{ width: '100%' }} animate={{ width: `${timerPct}%` }} transition={{ duration: 1, ease: 'linear' }} />
                </div>
              </div>

              {/* Question */}
              <motion.div className="w-full bg-slate-900/90 backdrop-blur-2xl rounded-3xl p-8 mb-6 flex items-center justify-center border-4 border-cyan-400/80 shadow-2xl text-center min-h-[180px]">
                <h2 className="font-sans font-extrabold text-xl md:text-2xl text-white leading-relaxed">
                  {currentQ.question}
                </h2>
              </motion.div>
                ...
                {currentQ.options.map((opt, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full py-5 px-6 rounded-2xl text-base font-extrabold font-sans transition-all duration-150 border-b-6 cursor-pointer text-left active:border-b-0 active:translate-y-2.5 shadow-xl ${
          ...
          {gameState === 'stage_victory' && (
            <motion.div key="stage_victory" className="w-full max-w-md bg-slate-900/95 backdrop-blur-2xl border-4 border-amber-400 rounded-3xl p-8 shadow-2xl flex flex-col items-center text-center font-sans text-white">
              ...
              <p className="text-xs text-cyan-200 mb-6 font-bold">Open Trivia dueli muvaffaqiyatli yakunlandi!</p>
          ...
          {gameState === 'gameover' && (
            <motion.div key="gameover" className="w-full max-w-md bg-slate-900/95 backdrop-blur-2xl border-4 border-rose-500 rounded-3xl p-8 shadow-2xl flex flex-col items-center text-center text-white font-sans">
              ...
              <p className="text-xs text-slate-400 mb-6">Yana bir bor bilimingizni sinab ko'ring!</p>
              <div className="flex flex-col gap-3 w-full">
