...
const newPromptFn = `    const aiSchema = {
      type: "object",
      properties: {
        questions: {
          type: "array",
...
const promptRegex = /\/\/\s*═══════════════════════════════════════════════════════════════[\s\S]*?const prompt = buildTestPrompt\([^)]+\);/;
content = content.replace(promptRegex, newPromptFn);

// Replace Groq call: body
content = content.replace(
  /body:\s*JSON\.stringify\(\{[\s\S]*?response_format:\s*\{\s*type:\s*"json_object"\s*\}\s*\/\/\s*Groq JSON format\s*\}\)/,
  `body: JSON.stringify({
                  model: task.model,
                  messages: [
...
// Replace Gemini call
content = content.replace(
  /const model = genAI\.getGenerativeModel\(\{[\s\S]*?generationConfig: \{ responseMimeType: 'application\/json' \}\s*\}\);/,
  `const model = genAI.getGenerativeModel({
                model: task.model,
                generationConfig: { responseMimeType: 'application/json', temperature: 0.2, responseSchema: aiSchema },
...
// Add mapping of correctAnswerIndex to correctOption in the parsing block
// Original parsing block:
...
content = content.replace(
  /let questions = \[\];\s*if \(Array\.isArray\(parsedObj\)\) \{\s*questions = parsedObj;\s*\}\s*else if \(parsedObj\.questions && Array\.isArray\(parsedObj\.questions\)\) \{\s*questions = parsedObj\.questions;\s*\}\s*else \{\s*questions = \[parsedObj\];\s*\}/,
  `let questions = [];
            if (Array.isArray(parsedObj)) {
              questions = parsedObj;
            } else if (parsedObj.questions && Array.isArray(parsedObj.questions)) {
