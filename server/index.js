...
<<<<<<< Updated upstream
    ...
    try {
      const subs = await TelegramSubscription.find({ studentName: new RegExp('^' + data.studentName + '$', 'i') });
=======

    // Telegram Auto-Broadcast
    try {
      const subs = await TelegramSubscription.find({ studentName: new RegExp('^' + data.studentName + '
        ...
        for (const update of data.result) {
          ...
          if (update.message && update.message.text) {
            ...
            if (text.startsWith('/obuna')) {
              ...
              if (parts.length > 1) {
                const nameToSub = parts.slice(1).join(' ');
                try {
...
>>>>>>> Stashed changes
      if (subs && subs.length > 0) {
        const isPass = data.totalScore >= 70;
        const statusEmoji = isPass ? '🟢' : '🔴';
        const msg = `🔔 <b>YANGI NATIJA QO'SHILDI</b> 🔔\n\n👤 <b>O'quvchi:</b> ${data.studentName}\n📊 <b>Natija:</b> <b>${data.totalScore}/100 ball</b> ${statusEmoji}\n\n🔗 <a href="https://bmdiagnostika.vercel.app/summary/${data.id}">Batafsil Hisobotni Ko'rish</a>`;
